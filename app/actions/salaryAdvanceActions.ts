...
import { handleApiResponse, createServerAction, createAuthHeaders } from '@/lib/actions';
...
export const createAdvanceAction = createServerAction(
  async (data: CreateAdvanceDto) => {
    ...
    const result = await handleApiResponse<SalaryAdvance>(response, 'Error creating salary advance');
...
export const getAllAdvancesAction = createServerAction(
  async (filters: SalaryAdvanceFilters = {}) => {
    ...
    const url = `${API_BASE_URL}/api/salary-advances${params.toString() ? '?' + params.toString() : ''}`;
    ...
    return handleApiResponse<SalaryAdvanceListResponse>(response, 'Error fetching salary advances');
  },
...
export const getMyAdvancesAction = createServerAction(
  async () => {
    ...
    const response = await fetch(`${API_BASE_URL}/api/salary-advances/employee`, {
      method: 'GET',
      headers,
    });

    return handleApiResponse<SalaryAdvance[]>(response, 'Error fetching your salary advances');
  },
  'Failed to fetch your salary advances'
);

// Approve salary advance - matches backend: PATCH /api/salary-advances/update/:id
export const approveAdvanceAction = createServerAction(
  async (id: number) => {
    const headers = await createAuthHeaders();
    
    const response = await fetch(`${API_BASE_URL}/api/salary-advances/update/${id}`, {
      method: 'PATCH',
      headers,
      body: JSON.stringify({ status: 'approved' }),
    });

    return handleApiResponse<SalaryAdvance>(response, 'Error approving salary advance');
  },
  'Failed to approve salary advance'
);

// Reject salary advance - matches backend: PATCH /api/salary-advances/update/:id
export const rejectAdvanceAction = createServerAction(
  async (id: number) => {
    const headers = await createAuthHeaders();
    ...
    return handleApiResponse<SalaryAdvance>(response, 'Error rejecting salary advance');
  },
  'Failed to reject salary advance'
);

// Get single salary advance (if needed)
export const getAdvanceAction = createServerAction(
  async (id: number) => {
    const headers = await createAuthHeaders();
    ...
    return handleApiResponse<SalaryAdvance>(response, 'Error fetching salary advance');
  },
  'Failed to fetch salary advance details'
);

// Delete salary advance (only if pending)
export const deleteAdvanceAction = createServerAction(
  async (id: number) => {
    const headers = await createAuthHeaders();
    ...
    const result = await handleApiResponse<{ message: string }>(response, 'Error deleting salary advance');
